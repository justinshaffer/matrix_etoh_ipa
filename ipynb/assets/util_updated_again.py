...
def all_dists(table, rare_depth, tree, taxonomy, minf=None, rpca_depth=None):
    ...
    beta_res['phylo-RPCA'] = phylogenetic_rpca_with_taxonomy(table, tree, taxonomy, min_feature_count=0,
                            min_feature_frequency=0,
                            min_sample_count=rare_depth)
    return beta_res

def all_dists_no_tree(table, rare_depth, minf=None, rpca_depth=None):
...
def all_dists_metab(table, minf=None, rpca_depth=None):
    ...
    beta_res = {}
    beta_res['Jaccard'] = beta(table, 'jaccard')
    beta_res['Bray-Curtis'] = beta(table, 'braycurtis')
    beta_res['Cosine'] = beta(table, 'cosine')
    beta_res['Canberra'] = beta(table, 'canberra')
    beta_res['Canberra-Adkins'] = beta(table, 'canberra_adkins')
    if rpca_depth == None:
        rpca_depth = 0 #use raredepth
    if minf == None:
        minf = 10 #default
    beta_res['RPCA'] = rpca(table, min_feature_count=0,
                            min_sample_count=0)
...
def all_dists_jaccard(table, rare_depth):
